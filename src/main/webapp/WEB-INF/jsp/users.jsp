            ...
            <div class="view-box">
                ...
                <table class="table table-striped display" id="datatable">
                    ...
                    <c:forEach items="${users}" var="user">
                        ...
                        <tr <c:choose>
                            <c:when test="${user.enabled}">checked</c:when>
                            <c:otherwise> class="disabledUser" </c:otherwise></c:choose>>
                            <td><c:out value="${user.name}"/></td>
                            <td><a href="mailto:${user.email}">${user.email}</a></td>
                            <td>${user.roles}</td>
                            <td><input type="checkbox" onchange="updateUser(${user.id})"
                                       <c:if test="${user.enabled}">checked</c:if> id="${user.id}"/></td>
                            <td><fmt:formatDate value="${user.registered}" pattern="dd-MMMM-yyyy"/></td>
                            <td><a><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a></td>
                            <td><a onclick="deleteRow(${user.id})"><span class="glyphicon glyphicon-remove"
